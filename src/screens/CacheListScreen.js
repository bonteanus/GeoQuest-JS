...
export default function CacheListScreen() {
  ...
  // Fetch data on load and when the user pulls down to refresh
  async function loadCaches() {
    const fetchedCaches = await fetchLiveCaches();
    setCaches(fetchedCaches);
    setLoading(false);
    setRefreshing(false);
  }
  ...
  // The 'R' in CRUDL: Render each item
  const renderCacheItem = ({ item }) => (
    <Pressable
      style={({ pressed }) => [
        styles.card,
        { opacity: pressed ? 0.7 : 1 } // Syllabus compliant opacity feedback!
      ]}
      onPress={() => console.log('Navigate to details: ', item.cacheId)}
    >
    ...
    <View style={styles.container}>
      <View style={styles.headerRow}>
        ...
        <Pressable
          ...
          onPress={() => console.log('Navigate to Create Cache')} 
        >
...
const styles = StyleSheet.create({
  ...
  createButton: { backgroundColor: '#4CAF50', borderRadius: 8, paddingHorizontal: 14, paddingVertical: 8 },
  createButtonText: { color: '#fff', fontWeight: 'bold', fontSize: 14 },
  list: { padding: 24 },
  card: { backgroundColor: '#1e2d3d', borderRadius: 12, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: '#2e4057' },
  cardTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  cacheName: { fontSize: 16, fontWeight: 'bold', color: '#fff', flex: 1 },
  cacheDescription: { color: '#aaa', fontSize: 14, marginBottom: 12, lineHeight: 20 },
  cardBottom: { flexDirection: 'row', justifyContent: 'flex-end' },
  points: { color: '#4CAF50', fontSize: 13, fontWeight: 'bold' },
